    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, (input_tensor, target) in enumerate(self.train_data_loader_weak):
            ...
            for i in range(ch):
                ...
                loss1,_ = self.criterion(output1_vec, target_vec)
                idx1_sorted = torch.argsort(loss1)
                loss2,_ = self.criterion(output2_vec, target_vec)
                idx2_sorted = torch.argsort(loss2)
                loss_tmp1 = loss1[idx2_sorted[:num_remember]].mean()
                loss_tmp2 = loss2[idx1_sorted[:num_remember]].mean()
                per_ch_score1_tmp = compute_per_channel_dsc(torch.nn.Sigmoid()(output1_vec[idx2_sorted[:num_remember]]),
                                                        target_vec[idx2_sorted[:num_remember]],mode="train")
                per_ch_score2_tmp = compute_per_channel_dsc(torch.nn.Sigmoid()(output2_vec[idx1_sorted[:num_remember]]),
                                                        target_vec[idx1_sorted[:num_remember]],mode="train")
                loss_ct_1=loss_ct_1+loss_tmp1
                loss_ct_2=loss_ct_2+loss_tmp2
                per_ch_score1.append(per_ch_score1_tmp.detach().cpu().numpy())
                per_ch_score2.append(per_ch_score2_tmp.detach().cpu().numpy())
            loss_ct_1=loss_ct_1/ch
            loss_ct_2=loss_ct_2/ch
            ...
            loss_last_1 = loss_ct_1 + (loss_consist_1 )/self.num_aug/ch * consistency_weight * 0.0009

            loss_last_2 = loss_ct_2 + (loss_consist_2 )/self.num_aug/ch * consistency_weight * 0.0009
