...
class Down_sample(nn.Module):
    def __init__(self, dropout_p, kernel_size=2, stride=2):
...
class Dense_Unet(BaseModel):
    ...
    def forward(self, x, mode="weak"):
        ...
        x = self.conv1(x)
        if mode == "strong":
            x, y1 = self.down1(self.d1(x), dropoutflag=True)
            x, y2 = self.down1(self.d2(x), dropoutflag=True)
            x, y3 = self.down1(self.d3(x), dropoutflag=True)
            x, y4 = self.down1(self.d4(x), dropoutflag=True)
        if mode == "weak":
